...
const usersearch = (req, res) => {
  ...
  dbcon.query(query, [clinnumber], (err, result) => {
    if (err) {
      ...
      let query =
        "SELECT * FROM rptowner inner join gender on rptowner.Gender_Id = gender.Gender_Id inner join address on rptowner.Address_Id = address.Address_Id where rptowner.Parent_Global_Id = ?";
